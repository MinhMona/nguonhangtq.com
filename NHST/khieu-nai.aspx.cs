...
namespace NHST
{
    public partial class khieu_nai1 : System.Web.UI.Page
    {
        ...
        public void LoadData()
        {
            ...
            {
                OrderID = Request.QueryString["ac"].ToString().Trim();
                search_id_new.Text = OrderID;
            }
            int page = 0;
            ...
            var la = ComplainController.GetListUserBySQL_searchid(u.ID,OrderID, page, 10);
            int total = ComplainController.GetTotalUser_searchid(u.ID,OrderID);
            pagingall(la, total);
        }
        //public void LoadData()
        //{
        ...
        protected void btnSearch_Click(object sender, EventArgs e)
        {
            ...
            string OrderID = search_id_new.Text.Trim();
            if (string.IsNullOrEmpty(OrderID) == true)
            {
                Response.Redirect("khieu-nai");
            }
            else
            {
                Response.Redirect("khieu-nai?ac=" + OrderID);
            }
        }
        #region Paging
        public void pagingall(List<tbl_Complain> acs, int total)
        {
            ...
            if (total > 0)
            {
                ...
                for (int i = 0; i < acs.Count; i++)
                {
                    ...
                    ltr.Text += "   <td>" + stt + "</td>";
                    ltr.Text += "     <td> <a href=\"/chi-tiet-don-hang/" + item.OrderID + "\" class=\"viewmore-orderlist\">Chi tiết</a></td>";
                    ltr.Text += "</tr>";
                }
            }
        }
