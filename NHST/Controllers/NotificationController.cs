...
namespace NHST.Controllers
{
    public class NotificationController
    {
        ...
        public static List<tbl_Notification> GetAllByNotifType(int ReceivedID)
        {
            using (var dbe = new NHSTEntities())
            {
                List<tbl_Notification> n = new List<tbl_Notification>();
                n = dbe.tbl_Notification.Where(no => no.ReceivedID == ReceivedID).OrderByDescending(no => no.ID).ToList();
                return n;
            }
        }
