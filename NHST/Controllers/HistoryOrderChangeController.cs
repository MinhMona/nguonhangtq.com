...
namespace NHST.Controllers
{
    public class HistoryOrderChangeController
    {
        ...
        public static string Insert_TransportationID(int TransportationID, int UID, string Username, string HistoryContent, int Type, DateTime CreatedDate)
        {
            using (var dbe = new NHSTEntities())
            {
                dbe.Configuration.ValidateOnSaveEnabled = false;
                tbl_HistoryOrderChange h = new tbl_HistoryOrderChange();
                h.TransportationOrderID = TransportationID;
                h.UID = UID;
                h.Username = Username;
                h.HistoryContent = HistoryContent;
                h.Type = Type;
