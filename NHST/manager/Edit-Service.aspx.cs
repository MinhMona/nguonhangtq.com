...
namespace NHST.manager
{
    public partial class Edit_Service : System.Web.UI.Page
    {
        ...
        public void LoadData()
        {
            ...
            if (id > 0)
            {
                var cusb = ServiceController.GetByID(id);
                if (cusb != null)
                {
                    ViewState["NID"] = id;
                    txtTitle.Text = cusb.ServiceName;
                    txtSummary.Text = cusb.ServiceContent;
                    txtCustomerBenefitLink.Text = cusb.ServiceLink;
                    isHidden.Checked = Convert.ToBoolean(cusb.IsHidden);
                    pPosition.Value = cusb.Position;

                    if (!string.IsNullOrEmpty(cusb.ServiceIMG))
                    {
                        imgDaiDien.ImageUrl = cusb.ServiceIMG;
                    }
                    pPosition.Value = cusb.Position;
